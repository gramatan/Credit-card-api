...
В релиз не вошли секреты(обсуждалось на встрече) и PVC(шаблон готов: [volume.yaml](volume.yaml) и все константы добавлены в values.yaml, но ввиду проблем на кластере по добавлению PVC решил не рисковать.).

Для проверки после установки необходимо перенаправить порт 24001.

установка релиза из папки gran-cc:
```shell
helm install gran-cc gran-cc-0.1.0.tgz
```

Week8. SHIFT-779. Создать начальные манифесты для своего приложения.
