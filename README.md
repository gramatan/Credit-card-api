...
- Card_info(Как в основной, так и в дополнительных задачах) использует JSONB ввиду того, что в нём хранится информация без строгого описания полей, как единая атомарную сущность, вся работа с которой происходит вне БД. При формализации требований по структуре данных внутри JSONB - можно будет выделить её в отдельную таблицу.
...
- Управление временным штампом отдано БД.
